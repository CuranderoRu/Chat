...
//        putLog(thread, "onTimeOutSocket");
    ...
    private void handleNonAuthorizedMsg(ChatSocketThread newClient, String value){
        ...
        if (client != null){
            client.sendMsg("Duplicate client connection is not allowed");
    ...
    private void sendBroadCastMsg(String msg, boolean addTime){
        String msg_toLog = msg;
        //if (addTime) msg_toLog = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss").format(new Date()) + " " + msg;
        if (addTime) msg_toLog = dateFormat.format(System.currentTimeMillis()) + msg;
        for (int i = 0; i < clients.size(); i++) {
            ChatSocketThread client = (ChatSocketThread) clients.get(i);
            if(client.authorized()) client.sendMsg(msg_toLog);
        }
    }
